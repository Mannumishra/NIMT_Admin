...
const AllUsers = () => {
    ...
    const getAllQuery = async () => {
        try {
            const res = await axios.get("https://api.nimteducation.com/api/get-queryes");
            console.log(res)
            if (res.status === 200) {
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            const res = await axios.put(`https://api.nimteducation.com/api/update-status/${id}`, { status: newStatus });
            if (res.status === 200) {
